...
files["backend/services/ai_service.py"] = r'''
import os
import json
...
files["backend/database/db.py"] = r'''
import sqlite3
from pathlib import Path
...
files["backend/services/auth_service.py"] = r'''
...
files["backend/routes/auth_routes.py"] = r'''
...
files["backend/services/diagnostic_service.py"] = r'''
from database.db import get_connection
...
def calculate_time_minutes(academic_level: str, questions: list):
    advanced_count = sum(1 for q in questions if q.get("difficulty") == "avanzada" or q.get("requires_upload"))
    ...
    templates = [
        {
            ...
            "options": ["Concepto correcto", "Concepto no relacionado", "Procedimiento inverso", "Ninguna de las anteriores"],
        ...
        },
        {
            ...
            "options": ["Método adecuado", "Método no aplicable", "Ensayo aleatorio", "No se puede resolver"],
        ...
        {
            "question": f"Determine cuál de las siguientes afirmaciones sobre {selected_topic} es correcta.",
            "options": ["Afirmación correcta", "Afirmación incompleta", "Afirmación falsa", "Afirmación contradictoria"],
        ...
        },
        {
            ...
            "options": ["Resultado correcto", "Resultado con error de signo", "Resultado incompleto", "Resultado fuera de contexto"],
        ...
        {
            "question": f"Desarrolle un procedimiento de mayor dificultad relacionado con {selected_topic}. Puede subir o escribir su desarrollo.",
            "options": ["Procedimiento correcto", "Error de método", "Error algebraico", "No corresponde"],
...
def generate_ai_test(data):
    ...
    system_prompt = """
Eres un generador experto de diagnósticos matemáticos para una plataforma educativa llamada Mathmentor IA.
Debes crear tests para estudiantes de ingeniería hasta segundo año.
...
- Debe incluir preguntas básicas, medias o avanzadas según el nivel.
- Si hay pregunta avanzada, una puede requerir desarrollo o archivo.
...
- requires_upload true solo en pregunta avanzada.
...
def evaluate_diagnostic(data):
    ...
    cursor.execute(
        ...
        (user_id, academic_level, area, topic, score, total, mastery, risk_level, detected_errors, weak_subtopics, weekly_hours)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            data.user_id,
...
def create_kpi(data):
    ...
    cursor.execute(
        """
        INSERT INTO kpis
        (user_id, topic, area, initial_mastery, current_mastery, target_mastery, weekly_hours, estimated_weeks, risk_level, status, pending_topics)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            data.user_id,
...
files["backend/routes/diagnostic_routes.py"] = r'''
from fastapi import APIRouter
from pydantic import BaseModel
...
files["backend/services/chatbot_service.py"] = r'''
from database.db import get_connection
...
files["backend/routes/chatbot_routes.py"] = r'''
from fastapi import APIRouter
from pydantic import BaseModel
...
files["backend/services/learning_service.py"] = r'''
from database.db import get_connection
from services.ai_service import generate_ai_json
...
Genera rutas de estudio personalizadas en JSON.
No agregues texto fuera del JSON.
"""
...
Debe incluir módulos. Cada módulo debe tener:
...
files["backend/routes/learning_routes.py"] = r'''
from fastapi import APIRouter
from pydantic import BaseModel
...
files["backend/services/teacher_mode_service.py"] = r'''
from database.db import get_connection
from services.ai_service import generate_ai_json
...
files["backend/routes/teacher_mode_routes.py"] = r'''
...
files["backend/services/analytics_service.py"] = r'''
from database.db import get_connection
...
files["backend/routes/analytics_routes.py"] = r'''
from fastapi import APIRouter
...
files["backend/services/coach_service.py"] = r'''
import random
...
files["backend/routes/coach_routes.py"] = r'''
from fastapi import APIRouter
    ...
    <section id="libre" class="app-section">
      ...
      <div class="chat-container">
        <div id="chatMessages" class="chat-messages">
          <div class="bot-message">Hola, soy Mathmentor IA. Puedo resolver, explicar, generar ejercicios y revisar procedimientos.</div>
        </div>
...
function showSection(id) {
  ...
  const active = Array.from(document.querySelectorAll(".side-nav button"))
    .find(button => button.textContent.toLowerCase().includes(id));
...
files["frontend/js/dashboard.js"] = r'''
function kpiBar(kpi) {
...
files["frontend/js/diagnostic.js"] = r'''
...
files["frontend/js/learning.js"] = r'''
...
files["frontend/js/teacher-mode.js"] = r'''
...
async function generateTeacherExercises() {
  ...
  const response = await fetch(`${API_URL}/api/teacher-mode/exercises?topic=${encodeURIComponent(topic)}&difficulty=${difficulty}&quantity=${quantity}`);
  const result = await response.json();
...
async function generateMockTest() {
  ...
  const response = await fetch(`${API_URL}/api/teacher-mode/mock-test?topic=${encodeURIComponent(topic)}&difficulty=${difficulty}&quantity=6&duration=45`);
  const result = await response.json();
...
files["frontend/js/analytics.js"] = r'''
...
files["frontend/css/styles.css"] = r'''
* {
  margin: 0;
  padding: 0;
